...
type ActionType = 'prediction' | 'accuracy-check';
export function PredictModal({ isOpen, onClose, datasetName, datasetId }: PredictModalProps) {
  ...
  const handlePredict = async () => {
    if (!selectedModelId || (actionType === 'accuracy-check' && !columnName.trim())) return;
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.BASE_URL}/v1/push`,
        {
          modelId: selectedModelId,
          datasetId: datasetId,
          accuracy_check: actionType === 'accuracy-check',
          columnName: actionType === 'accuracy-check' ? columnName : undefined
        }
      );
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={actionType} onValueChange={(value: ActionType) => setActionType(value)}>
              ...
              <div className="flex items-center space-x-2 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
                <RadioGroupItem value="accuracy-check" id="accuracy-check" />
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button
              ...
              disabled={!selectedModelId || isPredicting || (actionType === 'accuracy-check' && !columnName.trim())}
              className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white"
            >
              {isPredicting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  {actionType === 'accuracy-check' ? 'Checking...' : 'Predicting...'}
                </>
              ) : (
                <>
                  {actionType === 'accuracy-check' ? (
                  ...
                  {actionType === 'accuracy-check' ? 'Check Accuracy' : 'Run Prediction'}
                </>
              )}
            </Button>
          </div>
