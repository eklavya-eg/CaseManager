...
import { Database, Upload, Eye, Download, Trash2, Zap } from 'lucide-react';
import { PredictModal } from '@/components/PredictModal';
import { useDatasets, useModels } from '@/hooks/use-store';
...
export default function DataUpload() {
  ...
  const { datasets, isLoadingDatasets, fetchDatasets } = useDatasets();
  ...
  const displayDatasets = datasets.length > 0 ? datasets : [];
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { dataset } = response.data as { dataset: Dataset };
      fetchDatasets();
  ...
  const handlePredictClick = (datasetId: string, datasetName: string) => {
    setPredictModal({
      isOpen: true,
      datasetId,
      datasetName,
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8">
            ...
            <table className="w-full">
              ...
              <tbody>
                {displayDatasets.map((dataset, index) => (
                  <tr key={dataset.id || index} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td className="py-4 px-4 font-medium text-slate-900">{dataset.name}</td>
                    <td className="py-4 px-4 text-slate-600">{'10 mb'}</td>
                    <td className="py-4 px-4 text-slate-600">{'10000'}</td>
                    <td className="py-4 px-4 text-slate-600">{dataset.uploadedAt}</td>
                    <td className="py-4 px-4">{getStatusBadge(dataset.status || 'processing')}</td>
                    <td className="py-4 px-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handlePredictClick(dataset.id || '', dataset.name)}
      ...
      <PredictModal
        ...
        datasetName={predictModal.datasetName}
        datasetId={predictModal.datasetId}
      />
    </div>
  );
}